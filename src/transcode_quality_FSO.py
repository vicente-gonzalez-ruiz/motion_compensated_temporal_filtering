...
# --------------------------------------------------------------------
def codestream_point(): # A single gop
    ...
    # Extraction to .yuv
    shell.run("(ffmpeg -y -s " + str(x_dim) + "x" + str(y_dim) + " -pix_fmt yuv420p -i L_0/%4d.Y gop" + str(gop+1) + ".yuv) > /dev/null 2> /dev/null")

    # PSNR
    # ------------------------
    p = sub.Popen("(snr --type=uchar --peak=255"
                + " --file_A=" + str(original_gop)
                + " --file_B=gop" + str(gop+1) + ".yuv"
...
def trace_selection(mode):
    if mode == -2:
        shell.run("echo \"\" > " + str(pwd) + "/selection.data")
    elif mode == -1:
        shell.run("echo \"--- EMPTY LAYER ---\" >> " + str(pwd) + "/selection.data")
    elif mode == 0:
        shell.run("echo \"\nGOP " + str(gop+1) + " from 0 to " + str(GOPs-1) + " GOPs\n\" >> " + str(pwd) + "/selection.data")
        shell.run("echo \"Angle = math.atan((psnr - old_psnr) / (kbps - old_kbps))\n\" >> " + str(pwd) + "/selection.data")
    elif mode == 1:
        shell.run("echo \"" + str(key) + ": " + str(try_layersub[key]) + "\t\t" + str("%.8f"%angle) + "\t= " + "(" + str("%.6f"%psnr) + " - " + str("%.6f"%old_psnr) + ") / (" + str("%.4f"%kbps) + " - " + str("%.4f"%old_kbps) + ")\" >> " + str(pwd) + "/selection.data")
    elif mode == 2:
...
def toFile():
    # {{{ Save to file
    shell.run("echo \"# KBPS #PSNR\" >> " + str(pwd) + "/curve_gop" + str(gop+1) + ".data")
    for z in range(0, len(FSO[gop])):
        shell.run("echo \"" + str(FSO[gop][z]) + "\" >> " + str(pwd) + "/selection_gop" + str(gop+1) + ".data")
        if z % 2 == 0:
            shell.run("echo \"" + str("%.3f"%float(FSO[gop][z]["kbps"])) + "\t" + str("%.3f"%float(FSO[gop][z]["psnr"])) + "\" >> " + str(pwd) + "/curve_gop" + str(gop+1) + ".data")
    # }}}
...
def toDirectory():
    ...
    shell.run("mv " + str(pwd) + "/*.data " + str(f))
    # }}}
...
def gop_video():
    ...
    original_gop = str(pwd) + "/original_gop" + str(gop+1) + ".yuv"
